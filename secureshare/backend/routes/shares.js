...
async function logDownload(shareId, req) {
  ...
  await db.query(
    `INSERT INTO download_logs (id, share_id, ip, user_agent, country, city, isp) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
    [uuidv4(), shareId, ip, userAgent, country, city, isp]
  );
}
...
router.post('/', auth, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const share = (await db.query('SELECT * FROM shares WHERE id = $1', [id])).rows[0];
...
router.get('/', auth, async (req, res) => {
  try {
    const shares = (await db.query('SELECT * FROM shares WHERE user_id = $1 ORDER BY created_at DESC', [req.user.id])).rows;
...
router.get('/:id/logs', auth, async (req, res) => {
  try {
    const share = (await db.query('SELECT * FROM shares WHERE id = $1 AND user_id = $2', [req.params.id, req.user.id])).rows[0];
    if (!share) return res.status(404).json({ error: 'SHARE_NOT_FOUND' });
    const logs = (await db.query(
      `SELECT id, ip, user_agent, country, city, isp, downloaded_at FROM download_logs WHERE share_id = $1 ORDER BY downloaded_at DESC`,
      [req.params.id]
    )).rows;
    res.json({ logs });
  } catch (err) {
...
router.get('/dl/:token', async (req, res) => {
  try {
    const share = (await db.query('SELECT * FROM shares WHERE token = $1', [req.params.token])).rows[0];
    if (!share) return res.status(404).json({ error: 'SHARE_NOT_FOUND' });
    ...
    await db.query('UPDATE shares SET download_count = download_count + 1 WHERE id = $1', [share.id]);
